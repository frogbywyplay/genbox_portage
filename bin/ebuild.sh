...
export SANDBOX_WRITE="${SANDBOX_WRITE}:/dev/shm:/dev/stdout:/dev/stderr:${PORTAGE_TMPDIR}:${BUILD_PREFIX}"
...
dyn_install() {
	...
	for f in ASFLAGS CATEGORY CBUILD CC CFLAGS CHOST CTARGET CXX \
		CXXFLAGS DEPEND EGIT_{BASE_URI,REPO_URI,REV,BRANCH,CUSTOM,GROUP} \
		EHG_{BASE_URI,REPO_URI,REVISION,BRANCH,CUSTOM} \
		EHG_MQ_{GUARDS,BRANCH,REVISION} EXTRA_ECONF \
		EXTRA_EINSTALL EXTRA_MAKE FEATURES INHERITED IUSE \
		LDFLAGS LIBCFLAGS LIBCXXFLAGS LICENSE LICENSE_CHOOSEN \
		PDEPEND PF PKGUSE PROVIDE PUBLIC RDEPEND RESTRICT SLOT \
